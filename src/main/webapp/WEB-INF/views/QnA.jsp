			...
			<div class="collapse-item">
				...
				<div id="${item.qna_no}" class="content">
					...
					<c:forEach var="map" items="${output1}" varStatus="status">	
						...
						<p class="ans"><img id=reply src="assets/image/reply.png">
					&nbsp<strong> ${map.answer_writer}</strong> :&nbsp&nbsp ${map.answer_content} &nbsp&nbsp&nbsp&nbsp- ${map.reg_date}</p>
						<br>
						</c:if>
						</c:forEach>
						<form action ="${pageContext.request.contextPath}/answer/add_ok.do" method="post">
						<div id=answerb>댓글: <input type="text" name="answer_content" />
						<input type="hidden" name="qna_no" value="${item.qna_no}" />
						<input  id= replybtn type="submit" value="답변 완료" /></div>
					</form>
